...
class bfyfb():
    def __init__(self, username, password, webdav_url, token=None):
        ...
        self.token = token if token is not None else "SCT228306TAAAASDDDD"  # 在此处设置默认token，这是假的
        self.beifen_msg = ""
    ...
    def serverchan(self, title, msg="", token=None):
        if token is None:
            token = self.token  # 使用类实例中的默认token
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--username", default="user", help="WebDAV server username")
